...
def _print_table(rows):
    ...
    data = [
        [str(r.id), r.company, r.role, r.status, r.applied_date, r.last_update, str(r.source or ""), str(r.notes or "")]
        for r in rows
    ]
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="jobcli", description="Track job applications from the terminal.")
    ...
    s.add_argument("--source", default=None, help="Where you found the job (LinkedIn, Indeed, referral...)")
    s.add_argument("--status", default="applied", help="applied|interview|offer|rejected|ghosted|withdrawn")
    s.add_argument("--applied-date", dest="applied_date", default=None, help="YYYY-MM-DD (defaults to today)")
